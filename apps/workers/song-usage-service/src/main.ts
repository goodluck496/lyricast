...
import { SongUsageDomainModule } from '@lyri-cast/song-usage-domain';
...
const log = new Logger('SongUsageWorker');

async function bootstrap() {
  try {
    const expressApp = express();
    const adapter = new ExpressAdapter(expressApp);
    const app = await NestFactory.create(SongUsageDomainModule, adapter, {
