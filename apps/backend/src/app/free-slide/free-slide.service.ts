...
export class FreeSlideService {
  private readonly assetsPath = path.resolve(__dirname, 'assets', 'complete-jsons', 'free-slides');
  ...
  getSlide(id: string): FreeSlide | { empty: 'not-found-in-file' | 'error-parse' } {
  ...
  private _writeJsonFile(filePath: string, data: Record<string, FreeSlide>): boolean {
    try {
