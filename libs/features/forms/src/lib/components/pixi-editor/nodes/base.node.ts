...
import { CursorType, HandleName } from '../enums';
...
export abstract class NodeBase extends Container {
  ...
  setSelected(isSelected: boolean): void {
    ...
    if (
      ...
      !this.handlesContainer.destroyed
    ) {
  ...
  drawFrame(): void {
    ...
    if (!graphics || graphics.destroyed || typeof graphics.clear !== 'function')
      return;
  ...
  drawHandles(initialize = false): void {
    ...
    for (const handleName of handleNames) {
      ...
      const graphicsWithMeta =
        handleGraphics as unknown as GraphicsWithMetadata;
      ...
      if (handleName === 'rot') {
        ...
        graphicsWithMeta.cursor =
          handleName === 'n' || handleName === 's'
            ? 'ns-resize'
            : handleName === 'e' || handleName === 'w'
            ? 'ew-resize'
            : handleName === 'ne' || handleName === 'sw'
            ? 'nesw-resize'
            : 'nwse-resize';
      }
  ...
  getWorldScale(): { scaleX: number; scaleY: number } {
    const m = this.handlesContainer.worldTransform;
    return {
  ...
  getWorldCenter(): { x: number; y: number } {
    const p = this.handlesContainer.toGlobal({
      x: this.w / 2,
      y: this.h / 2,
    });
    return { x: p.x, y: p.y };
